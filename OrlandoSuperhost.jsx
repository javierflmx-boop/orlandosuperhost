...
/* ─────────────────────────────────────────────────────────
   🎨 YOUR BRAND COLORS — change here if needed
───────────────────────────────────────────────────────── */
const ORANGE   = "#E8601C";
const BG       = "#111111";
const CARD     = "#1A1A1A";
...
/* ─────────────────────────────────────────────────────────
   🚗 YOUR CARS — add as many as you want
   img: paste a direct photo URL or leave "" for solid color
───────────────────────────────────────────────────────── */
const CARS = [
  // { name: "Your Car Name", year: "2024", img: "https://..." },
];

/* ─────────────────────────────────────────────────────────
   🏡 YOUR HOMES — add each property
───────────────────────────────────────────────────────── */
const HOMES = [
  // { name: "Property Name", area: "Neighborhood", beds: 4, baths: 3, img: "https://..." },
];

/* ─────────────────────────────────────────────────────────
   🔗 YOUR LINKS
───────────────────────────────────────────────────────── */
const TURO_URL   = "https://turo.com";       // replace with your Turo profile
const AIRBNB_URL = "https://airbnb.com";     // replace with your Airbnb profile
const WHATSAPP   = "https://wa.me/1407000000"; // replace with your number

/* ═══════════════════════════════════════════════════════
   APP — nothing below needs to change unless you want to
═══════════════════════════════════════════════════════ */
...
function Landing({ onGo }) {
  ...
  const Tile = ({ id, label, sub, style = {} }) => (
    ...
    >
      {/* subtle orange top bar on hover */}
      <div style={{ position:"absolute", top:0, left:0, right:0, height:3, background:ORANGE, opacity: hov===id ? 1 : 0, transition:"opacity .2s" }} />

      <div style={{ fontSize:9, letterSpacing:"0.35em", color:ORANGE, textTransform:"uppercase", marginBottom:8 }}>
        {sub}
      </div>
      <div style={{ fontSize:44, fontWeight:700, color:WHITE, letterSpacing:"-0.02em", lineHeight:1 }}>
        {label}
      </div>
      <div style={{ position:"absolute", bottom:22, right:22, fontSize:18, color:ORANGE, opacity: hov===id ? 1 : 0.3, transition:"opacity .2s" }}>
        →
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight:"100vh", background:BG }}>
      ...
      <div style={{ padding:"0 16px 40px", display:"flex", flexDirection:"column", gap:10 }}>
        {CARS.length === 0 ? (
          <EmptyState icon="🚗" text="Your cars will show here" sub="Add your vehicles in the CARS section of the code" />
        ) : (
          CARS.map((c, i) => (
            <CarCard key={i} car={c} />
          ))
...
function CarCard({ car }) {
  ...
  return (
    <div
      ...
      style={{ borderRadius:22, overflow:"hidden", position:"relative", height:155, background:CARD, cursor:"pointer",
        transform: hov?"scale(0.985)":"scale(1)", transition:"transform .22s",
        boxShadow: hov?`0 0 0 1.5px ${ORANGE}`:"0 2px 10px rgba(0,0,0,.4)" }}
    >
      {car.img ? (
...
function HomesPage({ onBack }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:BG }}>
      ...
      <div style={{ padding:"0 16px 40px", display:"flex", flexDirection:"column", gap:10 }}>
        {HOMES.length === 0 ? (
          <EmptyState icon="🏡" text="Your homes will show here" sub="Add your properties in the HOMES section of the code" />
        ) : (
          HOMES.map((h, i) => (
            <HomeCard key={i} home={h} onAvail={() => setModal(h)} />
          ))
        )}
      </div>
...
function HomeCard({ home, onAvail }) {
  ...
  return (
    <div
      onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ borderRadius:22, overflow:"hidden", background:CARD, cursor:"pointer",
        transform: hov?"scale(0.985)":"scale(1)", transition:"transform .22s",
        boxShadow: hov?`0 0 0 1.5px ${ORANGE}`:"0 2px 10px rgba(0,0,0,.4)" }}
    >
      <div style={{ position:"relative", height:155 }}>
        {home.img ? (
          ...
          <div style={{ width:"100%", height:"100%", background:"linear-gradient(135deg,#1e1e1e,#2a2a2a)", display:"flex", alignItems:"center", justifyContent:"center" }}>
